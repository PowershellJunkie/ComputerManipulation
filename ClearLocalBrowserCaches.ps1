...
""
Write-Host -ForegroundColor Green "Visual Powershell tool to delete cache & cookies in Firefox, Chrome, IE and Edge (Chromium) browsers"
Write-Host -ForegroundColor Green "Original by Lee Bhogal, Remix (v.3.0+) by PowershellJunkie"
Write-Host -ForegroundColor Green "VERSION: 3.0"
""
Write-Host -ForegroundColor yellow "#######################################################"
""
Write-Host -ForegroundColor Green "CHANGE_LOG:
v3.0: - Added support for Edge Chromium, added line to remove users CSV when script completes, added lines to remove 'Snapshots'
...
if ($list) {
    ...
    Import-CSV -Path C:\users\$env:USERNAME\users.csv -Header Name | foreach {
			...
			Remove-Item -Path "C:\Users\$($_.Name)\AppData\Local\Google\Chrome\User Data\Snapshots\*" -Recurse -Force -EA SilentlyContinue -Verbose
			Remove-Item -path "C:\Users\$($_.Name)\AppData\Local\Google\Chrome\User Data\Default\Default\Code Cache\js\*" -Recurse -Force -EA SilentlyContinue -Verbose
			Remove-Item -path "C:\Users\$($_.Name)\AppData\Local\Google\Chrome\User Data\Default\Default\Cache\*" -Recurse -Force -EA SilentlyContinue -Verbose
			Remove-Item -path "C:\Users\$($_.Name)\AppData\Local\Google\Chrome\User Data\Default\Default\Service Worker\Cache Storage\*" -Recurse -Force -EA SilentlyContinue -Verbose
...
"-------------------"
    # Clear Edge Chromium 
    Write-Host -ForegroundColor Green "SECTION 6: Clearing Edge Chromium Caches"
    "-------------------"
    Write-Host -ForegroundColor yellow "Clearing Edge Chromium caches"
    Write-Host -ForegroundColor cyan
    Import-CSV -Path C:\users\$env:USERNAME\users.csv -Header Name | foreach {
    ...
    Write-Host -ForegroundColor yellow "Done..."
    ""
	# Remove users CSV now that task is completed
	Remove-Item -Path C:\users\$env:USERNAME\users.csv -Force -EA SilentlyContinue -Verbose
